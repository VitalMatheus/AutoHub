...
type Props = { initial?: Partial<CustomerInput>; submitting?: boolean; onSubmit: (value: CustomerInput) => Promise<unknown>; onCancel: () => void };
type Errors = Partial<Record<keyof CustomerInput, string>>;

function formatDocument(value: string) { return value.replace(/[^\d]/g, '').slice(0, 14); }
function formatPhone(value: string) { return value.replace(/[^\d()+\-\s]/g, '').slice(0, 30); }

export function CustomerForm({ initial, submitting, onSubmit, onCancel }: Props) {
  ...
  async function submit(event: FormEvent) {
    ...
    try { await onSubmit(Object.fromEntries(Object.entries(value).filter(([, field]) => field.trim() !== '')) as CustomerInput); setDirty(false); }
    catch (error) {
      if (error instanceof ApiError && error.problem.status === 409) setApiError('Já existe um Customer com este documento.');
      else if (error instanceof ApiError) setApiError(error.problem.detail || 'Não foi possível salvar o Customer.');
  ...
  return <form onSubmit={submit} className="space-y-6" noValidate>
    ...
    <div className="grid gap-5 md:grid-cols-2">
      ...
      <Field label="CPF/CNPJ" error={errors.document}><input value={value.document} onChange={(e) => update('document', formatDocument(e.target.value))} /></Field>
      ...
      <Field label="Observações" className="md:col-span-2"><textarea rows={4} value={value.notes} onChange={(e) => update('notes', e.target.value)} /></Field>
    </div>
