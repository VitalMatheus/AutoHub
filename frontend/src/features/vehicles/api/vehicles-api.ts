...
import type { WorkOrder } from '@/features/work-orders/api/work-orders-api';
...
export type VehicleWorkOrderList = { data: WorkOrder[]; meta: { page: number; pageSize: number; total: number; totalPages: number } };
...
export async function listVehicleWorkOrders(id: string, params: { page: number; pageSize: number }): Promise<VehicleWorkOrderList> { return (await httpClient.get<VehicleWorkOrderList>(`/vehicles/${id}/work-orders`, { params })).data; }
