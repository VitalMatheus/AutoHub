...
const faqs = [
  ...
  ['O que acontece depois dos 14 dias?', 'Você decide se quer continuar com o Basic Plan por R$ 79/mês. Não há cobrança automática durante o teste.'],
...
export function LandingPage() {
  return <div className="public-site"><PublicHeader /><main>
    ...
    <section id="precos" className="public-section"><div className="public-container grid items-center gap-10 md:grid-cols-[.8fr_1.2fr]"><div><p className="eyebrow dark">Um plano para começar</p><h2>Basic Plan</h2><p>Tenha todas as capacidades atuais para uma oficina e até três administradores.</p></div><div className="price-card"><p className="text-sm font-bold uppercase tracking-widest text-blue-700">Basic Plan</p><p className="price">R$ 79<span>/mês</span></p><p className="font-semibold text-slate-700">14 dias grátis. Depois, R$ 79/mês.</p><ul>{['Clientes e veículos sem limites artificiais', 'Orçamentos e ordens de serviço', 'Produtos, serviços, fornecedores e compras', 'Financeiro, recebimentos e relatórios'].map((item) => <li key={item}><Check size={18} aria-hidden="true" />{item}</li>)}</ul><CtaLink className="w-full" /></div></div></section>
...
const pageCopy: Record<string, { title: string; intro: string; body: string }> = {
  '/precos': { title: 'Preços simples e transparentes', intro: '14 dias grátis. Depois, R$ 79/mês.', body: 'O Basic Plan inclui as capacidades atuais do Vekar para sua oficina, sem taxa de implantação e sem fidelidade.' },
  '/termos': { title: 'Termos de Uso', intro: 'Informações sobre o uso do Vekar.', body: 'Esta página apresenta a versão pública dos termos do produto. Antes de contratar, consulte a versão vigente e tire dúvidas com nossa equipe.' },
  '/privacidade': { title: 'Política de Privacidade', intro: 'Transparência sobre seus dados.', body: 'O Vekar trata dados para oferecer a gestão da oficina, proteger o acesso e prestar suporte. Não venda seus dados e não usamos informações operacionais em eventos de funil.' },
  '/contato': { title: 'Fale com o Vekar', intro: 'Tem uma dúvida sobre o produto?', body: 'Estamos preparando os canais de atendimento da nova marca. Enquanto isso, acompanhe esta página para saber como falar com a equipe.' },
  '/teste-gratis': { title: 'Teste grátis por 14 dias', intro: 'Sem cartão e sem compromisso.', body: 'A entrada do cadastro público está sendo preparada. Você já pode conhecer o produto e conferir as condições do Basic Plan.' },
};
