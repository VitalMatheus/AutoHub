...
export function ProductForm({ initial, submitting = false, omitEmptyOptional = true, onSubmit, onCancel }: Props) {
  const [value, setValue] = useState<ProductInput>({ name: initial?.name ?? '', description: initial?.description ?? '', sku: initial?.sku ?? '', salePrice: initial?.salePrice ?? '' });
  ...
  async function submit(event: FormEvent) {
    ...
    const normalized = { ...value, name: value.name.trim(), description: value.description?.trim(), sku: value.sku?.trim() };
  ...
  return <form onSubmit={submit} className="space-y-5" noValidate>
    ...
    <div className="grid gap-5 md:grid-cols-2">
      ...
      <label className="space-y-1 text-sm font-medium">SKU<input aria-label="SKU" className={inputClass} value={value.sku} onChange={(e) => update('sku', e.target.value)} /></label>
