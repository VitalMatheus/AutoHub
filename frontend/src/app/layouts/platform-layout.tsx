...
export function PlatformLayout() {
  ...
  return <div className="min-h-screen bg-slate-100 text-slate-950">
    ...
    <aside id="platform-navigation" className={cn('fixed inset-y-0 left-0 z-40 flex w-72 flex-col bg-slate-950 text-slate-300 transition-transform lg:translate-x-0', open ? 'translate-x-0' : '-translate-x-full')} aria-label="Navegação da plataforma" role={open ? 'dialog' : undefined} aria-modal={open || undefined}>
      <div className="flex h-20 items-center justify-between border-b border-white/10 px-6"><Link to="/platform" className="flex items-center gap-3 text-white"><span className="grid size-9 place-items-center rounded-xl bg-violet-500 font-bold">A</span><span><strong className="block text-lg tracking-tight">AutoHub</strong><small className="text-xs text-slate-400">Administração da plataforma</small></span></Link>{open && <button ref={triggerRef} type="button" className="rounded-lg p-2 hover:bg-white/10 lg:hidden" onClick={() => setOpen(false)} aria-label="Fechar navegação"><X size={20} /></button>}</div>
