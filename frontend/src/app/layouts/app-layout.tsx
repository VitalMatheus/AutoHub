...
function Sidebar({ open, onClose, triggerRef }: { open: boolean; onClose: () => void; triggerRef: React.RefObject<HTMLButtonElement | null> }) {
  ...
  return <>
    ...
    <aside id="mobile-navigation" className={cn('fixed inset-y-0 left-0 z-40 flex w-72 flex-col bg-slate-950 text-slate-300 transition-transform lg:translate-x-0', open ? 'translate-x-0' : '-translate-x-full')} aria-label="Navegação principal" role={open ? 'dialog' : undefined} aria-modal={open ? true : undefined}>
      <div className="flex h-20 items-center justify-between border-b border-white/10 px-6"><Link to="/app/dashboard" className="flex items-center gap-3 text-white" onClick={onClose}><span className="grid size-9 place-items-center rounded-xl bg-blue-500 font-bold">V</span><span><strong className="block text-lg tracking-tight">Vekar</strong><small className="text-xs text-slate-400">Gestão de oficina</small></span></Link>{open && <button ref={closeButtonRef} type="button" className="rounded-lg p-2 text-slate-400 hover:bg-white/10 hover:text-white lg:hidden" onClick={onClose} aria-label="Fechar menu"><X size={20} /></button>}</div>
