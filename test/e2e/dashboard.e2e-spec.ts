...
describe('Commercial Dashboard (e2e)', () => {
  ...
  it('returns a consistent commercial snapshot to Super Admin only', async () => {
    ...
    expect(response.body).toEqual(expect.objectContaining({ referenceAt: '2026-01-20T12:00:00.000Z', timezone: 'America/Recife', organizations: expect.objectContaining({ total: expect.any(Number), current: expect.any(Number), dueSoon: expect.any(Number), overdue: expect.any(Number), paymentBlocked: expect.any(Number), suspended: expect.any(Number) }), financial: expect.objectContaining({ receivedRevenue: expect.stringMatching(/^\d+\.\d{2}$/), openWithinDue: expect.stringMatching(/^\d+\.\d{2}$/), overdue: expect.stringMatching(/^\d+\.\d{2}$/) }), attentionOrganizations: expect.any(Array) }));
