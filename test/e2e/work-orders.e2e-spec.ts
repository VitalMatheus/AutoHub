...
describe('Work Orders (e2e)', () => {
  ...
  afterAll(async () => { if (!prisma) return; await prisma.workOrderItem.deleteMany({ where: { organizationId: { in: [organizationId, otherOrganizationId] } } }); await prisma.workOrder.deleteMany({ where: { organizationId: { in: [organizationId, otherOrganizationId] } } }); await prisma.quoteItem.deleteMany({ where: { organizationId: { in: [organizationId, otherOrganizationId] } } }); await prisma.quote.deleteMany({ where: { organizationId: { in: [organizationId, otherOrganizationId] } } }); await prisma.vehicle.deleteMany({ where: { organizationId: { in: [organizationId, otherOrganizationId] } } }); await prisma.customer.deleteMany({ where: { organizationId: { in: [organizationId, otherOrganizationId] } } }); await prisma.user.deleteMany({ where: { organizationId: { in: [organizationId, otherOrganizationId] } } }); await prisma.organization.deleteMany({ where: { id: { in: [organizationId, otherOrganizationId] } } }); await app.close(); });
  ...
  it('creates direct Work Orders with atomic numbering and snapshots', async () => {
    ...
    const response = await request(app.getHttpServer()).post('/api/v1/work-orders').set('Authorization', `Bearer ${token}`).send({ customerId, vehicleId, organizationId: otherOrganizationId, reportedProblem: 'Noise', mileage: 12000, items: [{ type: 'SERVICE', serviceId: service.id, quantity: '1.500', unitPrice: '12.34' }, { type: 'MANUAL', description: 'Inspection', quantity: '2', unitPrice: '3.21' }] }).expect(201);
