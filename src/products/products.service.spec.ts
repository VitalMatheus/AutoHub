...
describe('ProductsService', () => {
  ...
  it('generates a SKU and exposes the low-stock status', async () => {
    ...
    expect(prisma.product.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ sku: expect.stringMatching(/^PROD-[A-Z0-9]{8}$/), stockQuantity: 2, stockMinimum: 3 }) }));
  ...
  it('regenerates an automatic SKU when the database reports a generated collision', async () => {
    ...
    expect(prisma.product.create.mock.calls[0][0].data.sku).toMatch(/^PROD-[A-Z0-9]{8}$/);
    expect(prisma.product.create.mock.calls[1][0].data.sku).toMatch(/^PROD-[A-Z0-9]{8}$/);
