  ...
  async workshopSummary(principal: AuthenticatedPrincipal, asOf = new Date()) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      const [organization, customers, vehicles, quotes, workOrders, payments, expenses, directSales, subscription, recentCustomers, recentVehicles, recentQuotes, recentWorkOrders] = await Promise.all([
        ...
        tx.quote.count({ where: { organizationId, status: { in: ['DRAFT', 'PENDING'] } } }),
        tx.workOrder.count({ where: { organizationId, status: { in: ['OPEN', 'WAITING_APPROVAL', 'IN_PROGRESS', 'WAITING_PARTS'] } } }),
