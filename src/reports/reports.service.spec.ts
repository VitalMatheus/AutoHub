...
describe('ReportsService', () => {
  it('keeps confirmed cash movements separate from receivables and payables', async () => {
    const prisma: any = {
      ...
      workOrder: { findMany: jest.fn().mockResolvedValue([{ id: 'wo', number: 1, status: 'COMPLETED', items: [{ type: 'SERVICE', quantity: d('1'), unitPrice: d('200.00') }], payments: [{ amount: d('100.00') }], stockAllocations: [] }]) },
