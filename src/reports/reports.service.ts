...
export class ReportsService {
  ...
  async managerial(principal: AuthenticatedPrincipal, query: ManagerialReportQueryDto = {}) {
    ...
    const [payments, salePayments, expensePayments, workOrders, sales, expenses] = await Promise.all([
      ...
      this.prisma.workOrder.findMany({ where: { organizationId, status: { not: 'CANCELLED' } }, select: { id: true, number: true, status: true, items: { select: { type: true, quantity: true, unitPrice: true }, }, payments: { where: { organizationId, status: 'CONFIRMED' }, select: { amount: true, discount: true } }, stockAllocations: { select: { quantity: true, unitCost: true } } } }),
    ...
    const receivable = [
      ...workOrders.map((x) => ({ source: 'WORK_ORDER', id: x.id, number: x.number, balance: add(x.items.reduce((s, i) => add(s, cents(i.quantity).mul(i.unitPrice)), zero()), x.payments.reduce((s, p) => add(s, cents(p.amount).add(p.discount ?? zero()).neg()), zero())) })),
